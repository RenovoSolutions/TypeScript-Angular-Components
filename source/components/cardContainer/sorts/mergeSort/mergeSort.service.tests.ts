...
/// <reference path='mergeSort.service.ts' />
...
module rl.ui.components.cardContainer.sorts.mergeSort {
	...
	describe('mergeSort', () => {
		...
		it('should sort a set of items using a custom comparer', (): void => {
			...
			var result: ITestObject[] = mergeSort.sort(data, (a: ITestObject, b: ITestObject): __compareResult.CompareResult => {
				return a.value > b.value
					? __compareResult.CompareResult.greater
					: (a.value < b.value ? __compareResult.CompareResult.less : __compareResult.CompareResult.equal);
			});
