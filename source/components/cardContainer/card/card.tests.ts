...
module rl.ui.components.cardContainer.card {
	...
	import __parentChild = utilities.services.parentChildBehavior;
	...
	describe('CardController', () => {
		...
		describe('saveCard', (): void => {
			it('should return the save promise if save behavior exists', (): void => {
				...
				var childBehavior: ICardChildBehaviorMock = {
					saveCard: sinon.spy((): ng.IPromise<void> => { return $q.when(); }),
				};
				parentChild.registerChildBehavior(card.item, childBehavior);
	
				var promise: ng.IPromise<void> = card.saveCard();
